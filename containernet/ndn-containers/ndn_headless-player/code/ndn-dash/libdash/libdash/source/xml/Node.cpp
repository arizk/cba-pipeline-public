...
dash::mpd::SegmentSize*                 Node::ToSegmentSize     () const
{
    dash::mpd::SegmentSize *segmentSize = new dash::mpd::SegmentSize();
    std::vector<Node *> subNodes = this->GetSubNodes();


    if (this->HasAttribute("id"))
    {
        segmentSize->SetID(this->GetAttributeValue("id"));
    }
    if (this->HasAttribute("scale"))
    {
        segmentSize->SetScale(this->GetAttributeValue("scale"));
    }
    if (this->HasAttribute("size"))
    {
        segmentSize->SetSize(stoi(this->GetAttributeValue("size")));
    }
...
dash::mpd::Representation*                  Node::ToRepresentation      ()  const
{
    ...
    for(size_t i = 0; i < subNodes.size(); i++)
    {
        ...
        if (subNodes.at(i)->GetName() == "SegmentSize")
        {
            representation->SetSegmentSize(subNodes.at(i)->ToSegmentSize());        //mpd extended with segment sizes
            continue;
        }
        if (subNodes.at(i)->GetName() != "FramePacking" && subNodes.at(i)->GetName() != "AudioChannelConfiguration" && subNodes.at(i)->GetName() != "ContentProtection")
